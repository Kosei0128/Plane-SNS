...
export async function POST(request: NextRequest) {
  try {
    ...
    // 在庫チェックと確保
    const reservedAccounts: { item: OrderItem; accounts: { id: string; account_info: string }[] }[] = [];
    for (const item of items) {
      ...
      if (stockError || availableAccounts.length < item.quantity) {
        return NextResponse.json({ success: false, message: `在庫不足です: ${item.itemTitle}` }, { status: 400 });
      }
    ...
    if (profileError || !profile || (profile.credit_balance || 0) < totalAmount) {
      return NextResponse.json({ success: false, message: "残高が不足しています" }, { status: 400 });
    }
    ...
    if (failedUpdates.length > 0) {
      ...
      return NextResponse.json({ success: false, message: "注文処理中にエラーが発生しました。" }, { status: 500 });
    }

    // 3. ユーザー残高を更新
    const newBalance = profile.credit_balance - totalAmount;
    ...
    const purchasedAccountInfo = reservedAccounts.flatMap(({ accounts }) => 
      accounts.map(acc => acc.account_info)
    );
    ...
    return NextResponse.json({
      success: true,
      orderId: order.id,
      purchasedAccounts: purchasedAccountInfo,
      message: "注文が完了しました",
    });
    ...
    const itemIds = Array.from(
      new Set(
        orders.flatMap((order) => 
          order.order_items.map((item: OrderItemFromDB) => item.item_id)
        )
      )
    );
