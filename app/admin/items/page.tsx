                ...
                <div className="flex-1">
                  <div className="mb-2 flex items-center gap-3">
                    ...
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${
                      item.stock > 50 ? 'bg-green-100 text-green-700' :
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-500">在庫操作:</span>
                    <div className="flex gap-1">
                      <button
                        ...
                        onClick={async () => {
                          ...
                          if (amount && !isNaN(Number(amount))) {
                            const newStock = item.stock + Number(amount);
                            const token = localStorage.getItem("admin-session");
                            try {
                              const res = await fetch("/api/admin/items", {
                                method: "PUT",
                      ...
                      <button
                        type="button"
                        onClick={async () => {
                          const amount = prompt("削除する在庫数を入力してください:", "10");
                          if (amount && !isNaN(Number(amount))) {
                            const newStock = Math.max(0, item.stock - Number(amount));
                            const token = localStorage.getItem("admin-session");
                            try {
                              const res = await fetch("/api/admin/items", {
                                method: "PUT",
                      ...
                      <button
                        type="button"
                        onClick={async () => {
                          const newStock = prompt("在庫数を直接設定してください:", String(item.stock));
                          if (newStock && !isNaN(Number(newStock))) {
                            ...
                            try {
                              const res = await fetch("/api/admin/items", {
                                ...
                                body: JSON.stringify({ id: item.id, stock: Number(newStock) })
                              });
                              const data = await res.json();
                              if (data.success) {
                                alert(`在庫を${newStock}個に設定しました`);
                                fetchItems();
                              } else {
                                alert(data.message || "更新に失敗しました");
                              }
                        ...
                        className="rounded bg-blue-100 px-2 py-1 text-xs font-semibold text-blue-700 transition hover:bg-blue-200"
                      >
                        =設定
                      </button>
                    </div>
