...
export default function OrdersPage() {
    ...
    return (
      <main className="mx-auto w-full max-w-5xl px-6 py-16">
        <div className="rounded-lg bg-red-50 p-6 text-center text-red-700">
          ...
          <Link href="/auth/login" className="mt-4 inline-block text-brand-blue hover:underline">
            ログインページへ
          </Link>
        </div>
      </main>
  ...
  if (orders.length === 0) {
    return (
      <main className="mx-auto flex min-h-[60vh] w-full max-w-5xl flex-col items-center justify-center px-6 py-16">
        <Package className="mb-6 h-24 w-24 text-slate-300" />
        <h1 className="mb-2 text-2xl font-bold text-brand-blue">注文履歴がありません</h1>
        <p className="mb-6 text-slate-600">まだ商品を購入していません</p>
        <Link
          href="/"
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-6 py-16">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-brand-blue">注文履歴</h1>
        <p className="mt-2 text-slate-600">過去に購入した商品とアカウント情報を確認できます</p>
      </div>
                      ...
                      <div key={item.id} className="rounded-lg border border-slate-200 bg-white p-4">
                        ...
                        <div className="mb-4 flex gap-4">
                          <div className="relative h-20 w-20 flex-shrink-0 overflow-hidden rounded-lg bg-slate-100">
                            <Image
                              src={item.imageUrl || "https://images.unsplash.com/photo-1518770660439-4636190af475?w=200"}
                              alt={item.title}
                              fill
                              ...
                              return (
                                ...
                                >
                                  <div className="flex items-center justify-between gap-2">
                                    <p className="flex-1 break-all font-mono text-sm text-slate-800">
                                      {account}
                                    </p>
                                    <button
