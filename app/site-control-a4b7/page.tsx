...
export default function AdminLoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/site-control-a4b7/login", {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || `ログインに失敗しました。 (${res.status})`);
      }

      const data = await res.json();

      if (data.success) {
        // The API now sets a secure cookie, so we just need to redirect.
    ...
    <main className="flex min-h-screen items-center justify-center bg-gradient-to-br from-brand-blue to-brand-turquoise px-6">
      <div className="w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl">
        ...
        <div className="mt-6 rounded-lg bg-slate-50 p-4 text-xs text-slate-600">
          ...
          <p>管理者: Supabaseで作成した管理者アカウントのメール / パスワード</p>
          <p>※ メールアドレスが <code>NEXT_PUBLIC_ADMIN_EMAILS</code> に登録されている必要があります。</p>
        </div>
      </div>
    </main>
  );
