...
export default function AdminItemsPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.message || `更新に失敗しました。 (${res.status})`);
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(errorData.message || `削除に失敗しました。 (${res.status})`);
      }
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (!res.ok) {
                        const errorData = await res.json().catch(() => ({}));
                        throw new Error(errorData.message || `商品の作成に失敗しました。 (${res.status})`);
                      }
