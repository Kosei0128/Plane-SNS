...
export default function AdminUsersPage() {
  ...
  const loadUsers = useCallback(async () => {
    try {
      ...
      if (!res.ok) {
        ...
        console.error("Failed to load users:", errorData.message || res.statusText);
  ...
  const handleSaveBalance = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json().catch(() => ({}));
        throw new Error(errorData.message || `残高の更新に失敗しました。 (${res.status})`);
      }
      ...
      if (data.success) {
        ...
        alert(`エラー: ${data.message || "残高の更新に失敗しました"}`);
      }
    } catch (error) {
